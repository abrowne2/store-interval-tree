...
/// An iterator that checks both interval overlap and a predicate
pub struct IntervalTreeIteratorWithPredicate<'v, 'i, T: Ord, V, P>
where
    P: Fn(&V) -> bool,
{
    pub(crate) nodes: Vec<&'v Node<T, V>>,
...
impl<'v, 'i, T: Ord + 'i, V: 'v, P> Iterator for IntervalTreeIteratorWithPredicate<'v, 'i, T, V, P>
where
    P: Fn(&V) -> bool,
{
    type Item = Entry<'v, T, V>;
    ...
    fn next(&mut self) -> Option<Entry<'v, T, V>> {
        loop {
            ...
            if Interval::overlaps(node_ref.interval(), self.interval) && (self.predicate)(node_ref.value()) {
                return Some(Entry {
                    value: node_ref.value(),
                    interval: node_ref.interval(),
                });
